...
# 2. CONFIG
# =====================================================
GUARD_MODEL_ID    = "meta-llama/llama-prompt-guard-2-22m"   # input & output guard
GENERATION_MODEL  = "meta-llama/llama-4-scout-17b-16e-instruct"  # RAG generation
...
# ---- Guard thresholds ----
# Prompt Guard returns a classification label: SAFE / JAILBREAK / INJECTION (varies by model).
# We treat anything other than "SAFE" as blocked.
GUARD_BLOCKED_LABELS = {"JAILBREAK", "INJECTION", "HARMFUL", "UNSAFE"}
...
def run_prompt_guard(text: str, context_label: str = "input") -> Tuple[bool, str, str]:
    """
    Run llama-prompt-guard-2-22m on `text`.

    Returns:
        (is_safe: bool, label: str, raw_response: str)

    IMPORTANT — model constraints:
    • llama-prompt-guard-2-22m is a TEXT CLASSIFICATION model on Groq.
    • It only accepts a SINGLE user message — NO system prompt allowed.
    • Max input is ~512 tokens; for output guard we truncate to ~1500 chars.
    • The model natively returns one of: SAFE, JAILBREAK, INJECTION.
    """
    # Prompt Guard 22M has a hard ~512 token limit (~400 chars safe threshold).
    # For output guard we send only the first 400 chars — enough to catch
    # harmful content without exceeding the limit.
    MAX_GUARD_CHARS = 400
    text_to_check = text[:MAX_GUARD_CHARS] if len(text) > MAX_GUARD_CHARS else text

    # Single user message — no system prompt (required by this classification model)
    user_msg = text_to_check
    ...
    try:
        ...
        # Normalise — sometimes the model returns "SAFE." or "SAFE\n"
        label = re.sub(r"[^A-Z]", "", raw)
        if not label:
            label = "SAFE"
        is_safe = label not in GUARD_BLOCKED_LABELS
        return is_safe, label, raw
    except Exception as e:
        # If guard call fails we fail-open with a warning (log the error)
        # and let the main pipeline decide. Adjust to fail-closed if preferred.
        st.warning(f"⚠️ Guard ({context_label}) tidak dapat dijangkau: {e}. Melanjutkan tanpa guard.")
...
def answer_looks_incomplete(text: str) -> bool:
    ...
    if not stripped:
        return True
    incomplete_endings = (
        "link", "link produk", "alasan medis", "kandungan kunci",
        "harga", "dengan", "dan", "serta", "karena", "untuk",
        "http", "https://reviews.f",
    )
...
def clinical_score_boost(doc_text: str, concerns: List[str]) -> float:
    ...
    boost = 0.0
    beneficial = {
        "inflamed_acne": ["benzoyl peroxide", "salicylic acid", "bha", "azelaic acid", "niacinamide", "centella", "cica", "sulfur", "zinc"],
        "acne": ["salicylic acid", "bha", "niacinamide", "zinc", "tea tree", "azelaic acid", "centella", "cica"],
        "comedonal_acne": ["salicylic acid", "bha", "retinol", "glycolic acid", "lactic acid"],
        "hyperpigmentation": ["vitamin c", "niacinamide", "alpha arbutin", "arbutin", "azelaic acid", "tranexamic acid", "licorice", "retinol"],
        "dry_skin": ["hyaluronic acid", "glycerin", "ceramide", "panthenol", "squalane", "aloe", "centella", "cica"],
        "sensitive_skin": ["centella", "cica", "panthenol", "ceramide", "aloe", "hyaluronic acid", "oat", "allantoin"],
        "large_pores": ["salicylic acid", "bha", "niacinamide", "zinc", "retinol"],
        "combination_skin": ["hyaluronic acid", "glycerin", "niacinamide", "ceramide", "gel", "lightweight", "ringan"],
        "normal_skin": ["hyaluronic acid", "glycerin", "ceramide", "centella", "panthenol", "moisturizing", "hydrating"],
        "oily_skin": ["niacinamide", "salicylic acid", "bha", "zinc", "oil control", "sebum"],
    }
    risky = {
        "sensitive_skin": ["retinol", "aha", "bha", "glycolic acid", "lactic acid", "fragrance", "parfum", "alcohol denat"],
        "dry_skin": ["alcohol denat", "high alcohol", "strong exfoliating"],
        "normal_skin": ["retinol", "aha", "bha", "salicylic acid", "glycolic acid", "lactic acid", "exfoliating"],
        "inflamed_acne": ["bengkoang", "whitening only", "brightening only"],
    }
...
### Catatan
Silakan perluas budget, ubah kategori produk, atau jelaskan kondisi kulit dengan lebih spesifik.
{safety}
"""
...
def load_resources():
    ...
    for _, row in df.iterrows():
        ...
        docs.append({
            "source": "produk", "category": category, "product_name": name,
            "brand": brand, "price": price, "raw_price": raw_price, "url": url, "text": full,
        })
    ...
    if os.path.exists(JOURNAL_CSV_PATH):
        ...
        for _, row in journal_df.iterrows():
            ...
            if not text:
                continue
            docs.append({
                "source": "jurnal", "category": "", "product_name": "", "brand": "",
                "price": None, "raw_price": "", "url": "", "topic": topic,
                "source_file": source_file, "page": page, "chunk_id": chunk_id,
                "text": (
                    f"[SUMBER: JURNAL DERMATOLOGI] [TOPIK: {topic}] "
                    f"[FILE: {source_file}] [HALAMAN: {page}] [CHUNK: {chunk_id}] {text}"
                ),
            })
...
def build_context(docs: List[Dict[str, Any]]) -> str:
    return "\n\n".join(f"[Dokumen {i}]\n{shorten_text(d['text'])}" for i, d in enumerate(docs, 1))
...
- Untuk jerawat meradang/nyeri: prioritaskan kandungan anti-inflamasi atau acne treatment seperti salicylic acid, benzoyl peroxide, azelaic acid, sulfur, zinc, niacinamide, centella/cica jika tersedia.
...
- Untuk kulit sensitif: prioritaskan soothing dan barrier repair seperti centella/cica, panthenol, ceramide, aloe, hyaluronic acid.
- Untuk kulit kombinasi: pilih pelembap ringan yang menghidrasi tanpa terlalu oklusif.
- Untuk kulit normal/maintenance: jangan over-treatment. Hindari AHA/BHA/retinol kecuali user eksplisit meminta.
- Untuk pori-pori besar: jelaskan bahwa pori tidak bisa dikecilkan permanen.
- Jika pengguna hamil/menyusui: jangan rekomendasikan retinol, retinal, retinoid, tretinoin, adapalene, atau tazarotene.
- Jika pengguna mengalami iritasi setelah exfoliating toner: fokus pada barrier repair dan soothing.
- Jangan menyatakan bahwa niacinamide, hyaluronic acid, atau cleanser biasa meningkatkan sensitivitas UV.
- Gunakan dokumen bertanda [SUMBER: JURNAL DERMATOLOGI] sebagai dasar "Alasan Dermatologis Berdasarkan Jurnal".
""".strip()
...
3. Fokus hanya pada empat kategori: facial wash, toner, serum, moisturizer.
4. Tampilkan maksimal {jumlah_produk} produk yang paling relevan.
5. Jika data kurang dari {jumlah_produk}, tampilkan yang tersedia dan beri catatan keterbatasan data.
6. Pada "Kandungan Kunci", tulis bahan aktif, bukan tekstur produk.
7. Jangan memberi diagnosis medis profesional.
8. Jika link tidak tersedia di konteks, tulis "Link tidak tersedia dalam basis data".
9. Jangan menulis harga Rp0. Jika harga tidak ada, tulis "Harga tidak tersedia".
10. Jangan tampilkan proses berpikir internal. Tampilkan jawaban final saja.
11. Gunakan bahasa ringkas. Analisis maks 2 kalimat, alasan maks 1 kalimat per produk, Catatan maks 2 poin.
12. Jangan tampilkan produk discontinued.
13. Jangan menjanjikan hasil instan atau klaim "menyembuhkan permanen".
14. Buat satu bagian "Alasan Dermatologis Berdasarkan Jurnal" setelah daftar rekomendasi.
...
def generate_answer(
    ...
    answer = call_generation_model(user_query, context, jumlah_produk, budget, category, concerns)
...
# ---- Pipeline badge display ----
st.markdown(
    """
    <div style="margin-bottom: 1.5rem; text-align: center;">
        <span class="pipeline-badge">① Input Guard<br><small>Prompt Guard 22M</small></span>
        &rarr;
        <span class="pipeline-badge">② RAG + Generate<br><small>Llama 4 Scout 17B</small></span>
        &rarr;
        <span class="pipeline-badge">③ Output Guard<br><small>Prompt Guard 22M</small></span>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if st.button("Analisis dan Rekomendasikan", type="primary"):
    ...
    # ── Step 0: Basic rule-based validation ──────────────────────────
    ...
    # ── Step 1: INPUT GUARD ──────────────────────────────────────────
    with st.spinner("🛡️ Memeriksa keamanan input (Prompt Guard)..."):
        input_safe, input_label, input_raw = run_prompt_guard(user_input, context_label="input")

    if not input_safe:
        st.markdown(
            f"<div class='guard-box'>"
            f"<strong>🚫 Input Diblokir oleh Safety Guard</strong><br><br>"
            f"Pertanyaan kamu terdeteksi sebagai: <code>{input_label}</code>.<br>"
            f"GlowAI hanya melayani pertanyaan seputar skincare yang aman. "
            f"Silakan tulis ulang pertanyaanmu."
            f"</div>",
            unsafe_allow_html=True,
        )
        st.stop()
    ...
    # ── Step 2: RETRIEVAL ────────────────────────────────────────────
    with st.spinner("🔍 Mencari dokumen relevan (FAISS)..."):
    ...
    # ── Step 3: GENERATION ───────────────────────────────────────────
    with st.spinner("✨ Menyusun rekomendasi (Llama 4 Scout)..."):
        try:
            answer, model_used = generate_answer(
                user_input, context, jumlah_produk, budget, category, concerns,
            )
        except Exception as e:
    ...
    # ── Step 4: OUTPUT GUARD ─────────────────────────────────────────
    with st.spinner("🛡️ Memeriksa keamanan output (Prompt Guard)..."):
        output_safe, output_label, output_raw = run_prompt_guard(answer, context_label="output")

    if not output_safe:
        st.markdown(
            f"<div class='guard-box'>"
            f"<strong>🚫 Output Diblokir oleh Safety Guard</strong><br><br>"
            f"Respons yang dihasilkan terdeteksi berpotensi berbahaya: <code>{output_label}</code>.<br>"
            f"GlowAI tidak dapat menampilkan rekomendasi ini. "
            f"Silakan coba pertanyaan yang berbeda atau hubungi tim dukungan."
            f"</div>",
            unsafe_allow_html=True,
        )
        st.stop()
    ...
    st.markdown(
        ...
        f"Guard: {GUARD_MODEL_ID} &nbsp;&middot;&nbsp; "
        f"Generator: {model_used} &nbsp;&middot;&nbsp; "
        f"Input guard: <strong>{input_label}</strong> &nbsp;&middot;&nbsp; "
        f"Output guard: <strong>{output_label}</strong> &nbsp;&middot;&nbsp; "
        ...
        f"Dokumen: {len(docs)} &nbsp;&middot;&nbsp; Jurnal: {journal_count}"
        f"</div>",
        unsafe_allow_html=True,
    )

    with st.expander("Transparansi Retrieval & Pipeline (Explainable RAG)"):
        st.markdown("**Pipeline yang Digunakan**")
        st.write(f"- **Input Guard:** `{GUARD_MODEL_ID}` → label: `{input_label}`")
        st.write(f"- **Generator:** `{model_used}`")
        st.write(f"- **Output Guard:** `{GUARD_MODEL_ID}` → label: `{output_label}`")
        st.markdown("---")
        ...
        st.write(f"- **Jumlah dokumen konteks:** {len(docs)} | Produk: {product_count} | Jurnal: {journal_count}")
        st.markdown("---")
        ...
        for i, d in enumerate(docs, 1):
            ...
            if d.get("source") == "produk":
                explanation.append("relevansi klinis")
            if not explanation:
                explanation.append("relevansi semantik umum")

            meta_lines = [
                f"**Dokumen {i}**",
                f"- Base score: `{d.get('base_score', 0):.3f}` | Final score: `{d.get('score', 0):.3f}`",
            ...
            meta_lines.append(f"- Alasan dipilih: **{', '.join(explanation)}**")
